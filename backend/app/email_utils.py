...
def _owner_emails() -> list[str]:
    """Splits OWNER_EMAIL on commas, trims whitespace, drops empty entries."""
    if not OWNER_EMAIL:
        return []
    return [e.strip() for e in OWNER_EMAIL.split(",") if e.strip()]
...
def send_owner_email(subject: str, order) -> tuple[bool, str]:
    """
    Sends a notification email to the owner(s) about an order event.
    ...
    if not (recipients and SMTP_USER and SMTP_PASSWORD):
